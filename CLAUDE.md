...
- **Carte offre unique** (`p-hero-offer`-like, fond dark `linear-gradient(135deg,#161616...,#111)` — **actuellement en test face à une version claire**, voir note ci-dessous) : badge "✓ Essai 7j", prix en gros (`PRICING[role]`, `billing` state réutilisé), toggle Annuel/Mensuel (boutons directs, pas de lien "voir le tarif mensuel"), texte d'ancrage en gras/italique rôle-aware ("Moins cher qu'une séance avec un coach particulier..." / "...qu'un mois de logiciel de coaching classique...").
  - **Mensuel** : `Facturé mensuellement · soit {perDay}€/jour` (`p.monthly / 30`, arrondi 2 décimales) — plus de mention "résiliable à tout moment" ici, déplacée en caption sous le CTA sticky (voir plus bas).
  - **Annuel** : `Facturé annuellement · {annual}€/an · Soit {annualSavings}€ d'économie` (`annualSavings = monthly*12 - annual` — 49€ sportif, 409€ coach, calculé dynamiquement, jamais codé en dur).
...
- **Bande "+600 sportifs, coachs et clubs"** (`PAYWALL_AVATARS`) — déplacée ici depuis `paywall_form` (voir plus bas) et chiffre remonté de 300 à 600 : plus utile comme réassurance pendant la décision du prix qu'en fin de formulaire, qu'on cherche au contraire à alléger.
...
- CTA `Actions` avec nouveau `caption="Résiliable à tout moment."` affiché sous le bouton sticky (voir prop `caption?` ci-dessous).
- **Test en cours, non tranché** : la carte offre passée en fond sombre à la demande explicite de Gildas ("fais un test") pour comparer à la version claire d'origine — actuellement ce qui est en prod, mais pas encore validé comme choix final.

**`paywall_form` — simplifié au maximum** (Gildas : "on simplifie au max le form de paiement") :
- Bloc "+300 sportifs..." **retiré entièrement** (redondant avec la bande "+600" désormais sur `paywall_priming`, juste avant).
- Nouvel encart "⚡ Ton compte est prêt." (fond `rgba(212,64,0,.08)`) en haut de l'écran — ne reste plus que ce qui est réellement à faire ici : le paiement.
- Sous le récap prix/toggle, ligne de réassurance dynamique selon `billing` : *"On te prévient 2 jours avant la fin de l'essai."* (annuel) / *"Annulation possible à tout moment, sans justification."* (mensuel) — remplace l'ancien bloc avatars au même emplacement visuel.
