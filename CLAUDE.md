...
Remplace la rotation générique `FOCUS_DIST` pour 5 catégories de sport où de vraies séances "phares" nommées existent (`SPORT_CURRICULUM: Partial<Record<SportCategory, (dayCount: number) => Archetype[]>>`), chacune avec un modèle de programmation connu et des règles explicites données par Gildas plutôt qu'un système générique uniforme :
...
- **Sprint** (`selectSprint`) : Accélération/Vitesse max/Endurance de vitesse/Tempo/Circuit + 1 slot Renfo **toujours réservé** quel que soit le nombre de jours ; plyométrie intégrée **directement dans la banque d'exercices** des séances de vitesse pure (Accélération, Vitesse max), pas une séance à part.
- **Haltérophilie** (`selectHalterophilie`) : modèle par **palier de jours** explicite, pas une liste de priorité — la structure change selon N : ≤2j → Arraché+Épaulé-Jeté ; 3j → +Total ; 4j → Épaulé et Jeté **séparés** (Arraché/Épaulé/Jeté/Total) ; 5j+ → + Séance légère variantes.
- **Powerlifting** (`selectPowerlifting`) : rotation round-robin Squat/Bench/Deadlift, mais **deadlift plafonné à 2 séances/semaine** (fatigue lombaire/SNC) — squat et bench peuvent aller jusqu'à 3.
...
- Les 8 autres catégories (combat, fitness, cyclisme, natation, ski, aviron, gymnastique, autre) **n'ont pas de curriculum** — gardent le système générique `FOCUS_DIST`. Gildas a explicitement invité à en chercher d'autres ("etc... à chaque fois il faut que tu cherches les modèles connus") ; pas encore fait, à reprendre si demandé.
- **Haltérophilie ≠ Powerlifting** : avant ce chantier, les deux tombaient dans la même catégorie `"halterophilie"` (mélangeant arraché/épaulé-jeté avec squat/bench/deadlift) — corrigé sur correction explicite de Gildas, `"powerlifting"` est maintenant une `SportCategory` distincte avec sa propre banque `EXERCISES.powerlifting`.
- **Les règles universelles d'enchaînement RPE (Phase A2 + Phase B, voir bug #3 ci-dessous) s'appliquent aussi aux sports à curriculum** — décision explicite de Gildas après un premier essai où j'avais gaté ces phases pour les sports à curriculum (jugeant que la structure du curriculum devait primer). Correction de Gildas : *"même avec un sport il faut respecter les règles d'enchaînement... ce sont des principes universels les plus importants, la génération des programmes est basée dessus"*. En pratique : si le curriculum place deux jours "durs" calendairement consécutifs (ex. Arraché puis Épaulé-Jeté un lundi-mardi), la Phase B corrige quand même — le jour corrigé perd son nom/contenu d'archétype et retombe sur la banque générique `EXERCISES[category][type]` du sport. Avec des jours espacés (repos entre chaque séance, ex. Lun/Mer/Ven), le curriculum s'affiche intact ; avec des jours calendairement consécutifs, une partie retombe sur du contenu générique — compromis attendu, pas un bug.

### 4 bugs de cohérence trouvés en relisant la sortie réelle (pas anticipés au départ)
...
Déployé en 10 commits séparés sur `main` (`f95b967` périodisation de base, `80b0791` prescriptions dynamiques, `67ec817` régénération bibliothèque, `8ff489e` cohérence jour/type + plafond récup, `4bfc03c` anti-consécutifs + arrondi + pluriel, `242dee5` variété réelle par type + zéro répétition dos-à-dos, `260d572` unification en 3 paliers de RPE facile/modéré/dur, `d57297b` minimise le palier modéré + premier jour de semaine jamais facile, `5635cd0` modèle 6 semaines + curriculum sportif par archétypes + fix intensité dupliquée + régénération complète des 49 programmes).
