...
- **CTA sticky partout (2026-07-11)** : sur tous les steps, le(s) bouton(s) restent visibles en bas de viewport même si le contenu dépasse l'écran.
  ...
  - Pattern dark (steps value_program(_coach)/concept_autoreg/celebration dans OnboardingFlow.tsx/CelebrationScreen.tsx, autoreg_score(_coach) dans leurs composants dédiés) : même `position:sticky; bottom:0; margin:16px -20px -56px; padding:14px 20px 24px`, **sans background** (retiré le 2026-07-11 — le fondu vers `#161616` ne raccordait pas au fond dégradé diagonal `135deg` de `OnboardingBackground`, créant une bande sombre visible au lieu d'un fondu). Le lien "Retour" y est un texte discret (`rgba(255,255,255,.45)`), pas un bouton icône — ces steps n'ont qu'un seul CTA pleine largeur au-dessus. Le pattern clair, lui, garde son background (`#f1f0ee` = couleur plate de la page, fondu invisible donc jamais problématique).
