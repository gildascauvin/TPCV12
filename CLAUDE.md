...
### 3 bugs de cohérence trouvés en relisant la sortie réelle (pas anticipés au départ)
...
### Vérifié
`tsc --noEmit` + `npm run build` propres à chaque étape. Testé en local sur plusieurs combinaisons (12 sem/mixte/intermédiaire, 4 sem/volume/débutant/2j, 12 sem/compétition/débutant, 8 sem/technique/élite/6j) — alternance de forme, tapering différé au dernier bloc pour "compétition", test forcé à chaque semaine MRV, aucun écrasement anormal au plancher de difficulté. Testé spécifiquement avec **les 7 jours de la semaine sélectionnés** (cas qui expose le plus de collisions, tableau `FOCUS_DIST` lu intégralement) sur les 7 valeurs de `focus` : 0 répétition de type consécutif, technique jamais >3/10. Vérifié sur les 27 programmes régénérés en base (pas juste en local, dernière régénération après tous les correctifs) : 0 répétition de type consécutif, technique et récupération jamais >3/10.

Déployé en 9 commits séparés sur `main` (`f95b967` périodisation de base, `80b0791` prescriptions dynamiques, `67ec817` régénération bibliothèque, `8ff489e` cohérence jour/type + plafond récup, `4bfc03c` anti-consécutifs + arrondi + pluriel, `242dee5` variété réelle par type + zéro répétition dos-à-dos, `260d572` unification en 3 paliers de RPE facile/modéré/dur, `d57297b` minimise le palier modéré + premier jour de semaine jamais facile).
