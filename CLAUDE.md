...
3. **Variation insuffisante entre jours consécutifs** — découvert en 3 étapes successives en testant avec tous les jours de la semaine sélectionnés (le cas qui expose le plus de collisions, car `FOCUS_DIST`, un tableau à 7 positions, est alors lu intégralement au lieu d'être échantillonné en creux) : d'abord repéré que deux jours calendairement consécutifs pouvaient être tous les deux intensité/test (notamment le test forcé de fin de semaine MRV tombant juste après un jour déjà intensité) ; puis que certaines `FOCUS_DIST` ont des types identiques déjà adjacents dans leur définition d'origine (ex. `FOCUS_DIST.volume` commence par `["volume","volume",...]`) ; puis que deux jours faciles (récupération+technique) consécutifs posent le même problème dans l'autre sens (principe de variation). **Solution finale, unifiée** : `RPE_BUCKET` classe chaque type en 3 paliers (`recuperation`/`technique`→"easy", `volume`→"moderate", `intensite`/`test`→"hard") — une seule règle, jamais deux jours calendairement consécutifs dans le même palier, peu importe le type exact. Le remplacement est choisi pour éviter le palier des DEUX voisins (pas juste celui qui posait initialement problème), avec "volume" en échappatoire prioritaire (sauf si le palier à casser est déjà "moderate"). Passe de lissage répétée jusqu'à stabilisation (jusqu'à 5 itérations) — nécessaire car corriger un jour peut faire apparaître une nouvelle collision détectée seulement au passage suivant. Nécessite un tri calendaire des jours (`DAY_ORDER`, `sortedDays`) pour distinguer jours réellement consécutifs (repos entre les deux ou non).
...
Déployé en 8 commits séparés sur `main` (`f95b967` périodisation de base, `80b0791` prescriptions dynamiques, `67ec817` régénération bibliothèque, `8ff489e` cohérence jour/type + plafond récup, `4bfc03c` anti-consécutifs + arrondi + pluriel, `242dee5` variété réelle par type + zéro répétition dos-à-dos, `260d572` unification en 3 paliers de RPE facile/modéré/dur).
