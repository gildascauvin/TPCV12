...
const DEFAULT_DAYS_LARGE = [0, 1, 3, 4]; // 4 sessions/week if > 5 athletes
const DEFAULT_DAYS_SMALL = [0, 2, 4];    // 3 sessions/week otherwise
...
export default function WeekPreviewStepCoach({ sport, athleteCount, onNext }: Props) {
  const templates = getSessionTemplates(sport);
  const isLarge = athleteCount > 5;
  const displayDays = isLarge ? DEFAULT_DAYS_LARGE : DEFAULT_DAYS_SMALL;
  ...
  displayDays.forEach((d, i) => {
    ...
    sessionForDay[d] = tpl;
    diffForDay[d] = tpl[2]; // use base difficulty directly (intermediate reference)
  });

  const [selectedDay, setSelectedDay] = useState<number>(displayDays[0]);
  ...
  return (
    <div>
      <div style={{ fontSize: 22, fontWeight: 950, letterSpacing: "-0.04em", marginBottom: 4 }}>
        {getSportEmoji(sport)} Programme type pour tes sportifs
      </div>
      <div style={{ fontSize: 13, color: "#8a8f94", lineHeight: 1.45, marginBottom: 18 }}>
        Généré selon ton sport et tes objectifs de coaching
      </div>
      ...
      <div style={{ position: "sticky", bottom: 0, margin: "16px -18px -18px", padding: "14px 18px 20px", background: "linear-gradient(180deg,rgba(255,255,255,0),rgba(255,255,255,.92) 28%,#fff 50%)" }}>
        ...
        >
          Ajuste tes séances selon tes sportifs →
        </button>
      </div>
    </div>
  );
