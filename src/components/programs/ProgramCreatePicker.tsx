...
/* Écran racine du flux de création — remplace le menu ancré (dropdown) d'une itération
   ...
   soit ouvre directement le drawer suivant (Générer/Importer, ProgramCriteriaModal.tsx), soit
   agit immédiatement (Modèle → nouvel onglet, Vierge → saut direct dans le builder). */
...
const OPTIONS = [
  ...
  { key: "template", icon: "📚", label: "Utiliser un modèle", sub: "Bibliothèque ↗" },
  { key: "blank", icon: "📄", label: "Programme vierge", sub: "Semaine 100% vide" },
] as const;

export default function ProgramCreatePicker({ onClose, onGenerate, onImport, onTemplate, onBlank }: Props) {
